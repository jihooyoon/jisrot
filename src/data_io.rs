...
pub fn write_app_event_list_to_json(file_out: &PathBuf, app_event_list: &Vec<AppEvent>) -> anyhow::Result<()> {
...
pub fn write_total_stats_to_json(file_out: &PathBuf, total_stats: &TotalStats) -> anyhow::Result<()> {
    if let Some(parent) = file_out.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let file = std::fs::File::create(file_out)?;
...
pub fn write_merchant_data_to_json(file_out: &PathBuf, merchant_data_list: &MerchantDataList) -> anyhow::Result<()> {
    if let Some(parent) = file_out.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let file = std::fs::File::create(file_out)?;
